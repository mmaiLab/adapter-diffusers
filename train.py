...
import wandb
from adadiffusers import AdapterConfig
from adadiffusers.utils import (
    ExperimentConfig,
    TemplateDataset,
    TrainArgs,
    load_base_model,
...
def training_function(models, device, args, use_wandb, model_save_name=None):
    ...
    if args.train_text_encoder and args.train_vae:
        ...
        unet, vae, optimizer, train_dataloader = accelerator.prepare(
            unet, vae, optimizer, train_dataloader)
                ...
                with accelerator.accumulate(vae):
                    with accelerator.accumulate(text_encoder):
                        ...
                        if args.train_vae == 'on_rec':
                            latents_vae = vae.encode(
                                sample_image).latent_dist.sample()
                        ...
                        if args.train_vae:
                            ...
                            loss_vae = F.mse_loss(
                                reconst_image, sample_image, reduction='none').mean([1, 2, 3]).mean()
                            loss_vae.backward()
                            #del loss_vae
                            torch.cuda.empty_cache()
                        #loss = loss+ loss_vae
                        ...
                        if args.max_grad_norm:
                            accelerator.clip_grad_norm_(
                                param_to_update, args.max_grad_norm)
                        optimizer.step()
                        optimizer.zero_grad()
    ...
    if accelerator.is_main_process:
        ...
        if 'style' in args.run_name:
            if model_save_name:
                save_adapter(accelerator.unwrap_model(
                    unet), output_dir / model_save_name)
            else:
                save_adapter(accelerator.unwrap_model(unet),
                             output_dir / 'unet_adapter_style.pth')
        else:
            save_adapter(accelerator.unwrap_model(unet),
                         output_dir / 'unet_adapter.pth')

        if args.train_text_encoder:
            save_adapter(accelerator.unwrap_model(text_encoder),
                         output_dir / 'text_adapter.pth')

        if args.train_vae:
            save_adapter(accelerator.unwrap_model(vae),
                         output_dir / 'vae_adapter.pth')


def init_config():
    args = TrainArgs(
        instance_data_root='vgg_face2-master/samples/loose_crop (release version)/n000106')
...
def main():
    ...
    if config_files is not None:
        ...
        config = OmegaConf.load(
            init_config_path / f'configs_{args.device_num}.yml')
    ...
    if args.skip_instance:
        ...
        # specify a directory based on timestamp
        train_args.output_dir = next(model_path.glob(f'*{timestamp}*'))
    ...
    if len(config_files) > 1:  # if train both instance and style adapters
        ...
        train_args.output_dir = Path(
            train_args.output_dir) / train_args.run_name
        output_dir = train_args.output_dir
        image_output_dir = train_args.image_output_dir
        model_save_name = args.model_save_name

    train_args.image_output_dir = Path(
        str(train_args.output_dir).replace('model', 'sample'))

    configs = ExperimentConfig(
        train_args=train_args, adapter_config=adapter_config)

    modules = adapter_config.modules['transformer'] + \
        adapter_config.modules['resnet'] + adapter_config.add_modules
    modules = '_'.join(modules)

    train_args.output_dir.mkdir(parents=True, exist_ok=True)
    accelerate.notebook_launcher(training_function, args=(
        models, device, train_args, args.log_wandb, model_save_name), num_processes=1)
    with torch.no_grad():
        torch.cuda.empty_cache()
    ...
    if len(config_files) > 1:  # if train both instance and style adapters
        ...
        style_modules = adapter_config.modules['transformer'] + \
            adapter_config.modules['resnet'] + adapter_config.add_modules
        ...
        OmegaConf.save(configs, train_args.image_output_dir /
                       'configs_style.yml')
